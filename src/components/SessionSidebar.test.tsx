...
const mockSessions: SessionData[] = [
  ...
  },
  {
    ...
    total_cost_usd: 0,
    cost_budget_usd: 0,
    note: "fixing the login regression",
    last_output_preview: "Running tests...",
...
describe("SessionSidebar", () => {
  ...
  it("does not show total cost in footer when all costs are 0", () => {
    const zeroCostSessions = mockSessions.map((s) => ({
      ...s,
      total_cost_usd: 0,
    }));
    render(
      <SessionSidebar
        sessions={zeroCostSessions}
        activeId={null}
        onSelect={vi.fn()}
        onNewSession={vi.fn()}
        onKill={vi.fn()}
  ...
  it("shows cost/budget when budget is set", () => {
    const sessions = [
      { ...mockSessions[0], total_cost_usd: 2.5, cost_budget_usd: 5.0 },
    ];
    render(
      <SessionSidebar
        sessions={sessions}
  ...
  it("shows 'clear done' button when there are DONE sessions", () => {
    const doneSessions = [{ ...mockSessions[0], status: "DONE" as const }];
    render(
      <SessionSidebar
        sessions={doneSessions}
    ...
    expect(screen.getByTitle(/clear done/i)).toBeInTheDocument();
  });

  it("does not show 'clear done' when no DONE sessions", () => {
    render(
      <SessionSidebar
        sessions={mockSessions}
        activeId={null}
    ...
    expect(screen.queryByTitle(/clear done/i)).toBeNull();
  });

  it("calls onClearDone when 'clear done' button is clicked", () => {
    const doneSessions = [{ ...mockSessions[0], status: "DONE" as const }];
