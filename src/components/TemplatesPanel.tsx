...
export function TemplatesPanel({ open, onClose, onLaunch }: Props) {
  ...
  useEffect(() => {
    if (!open) return;
    invoke<Template[]>("list_templates").then(setTemplates).catch(() => setTemplates([]));
  }, [open]);

  if (!open) return null;

  const handleDelete = async (id: string) => {
    const updated = await invoke<Template[]>("delete_template", { templateId: id });
  ...
  return (
    ...
    >
      <div
        style={{ width: 520, background: "var(--bg-secondary)", border: "1px solid var(--border)", borderRadius: 8, overflow: "hidden", boxShadow: "0 24px 48px rgba(0,0,0,0.4)" }}
        onClick={(e) => e.stopPropagation()}
      >
        <div style={{ padding: "12px 16px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <span style={{ fontSize: 13, fontWeight: 600, color: "var(--text-primary)" }}>Session Templates</span>
          <span style={{ fontSize: 10, color: "var(--text-secondary)" }}>Save & relaunch session configs</span>
        </div>
          ...
          {templates.length === 0 ? (
            ...
            templates.map((t) => (
              <div key={t.id} style={{ padding: "10px 16px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", gap: 10 }}>
                <span style={{ fontSize: 14 }}>{AGENT_ICONS[t.agent_type] || "▸"}</span>
                ...
                <button
                  aria-label="launch"
                  onClick={() => { onLaunch(t); onClose(); }}
                  style={{ background: "var(--accent-blue)", border: "none", borderRadius: 4, color: "#000", cursor: "pointer", fontSize: 11, padding: "4px 10px", fontWeight: 600 }}
                >
                  Launch
                </button>
                <button
                  ...
                  style={{ background: "none", border: "1px solid var(--border)", borderRadius: 4, color: "var(--text-secondary)", cursor: "pointer", fontSize: 11, padding: "4px 8px" }}
                >
                  ✕
                </button>
              </div>
