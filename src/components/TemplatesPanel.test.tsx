...
const mockTemplates = [
  { id: "t1", name: "auth-worker", agent_type: "claude-code", working_dir: "/repo", count: 3 },
  { id: "t2", name: "quick-fix",   agent_type: "aider",        working_dir: null,     count: 1 },
];
...
describe("TemplatesPanel", () => {
  ...
  it("loads and displays templates", async () => {
    ...
    await waitFor(() => expect(screen.getByText("auth-worker")).toBeInTheDocument());
  ...
  it("calls onLaunch with template data when launch clicked", async () => {
    ...
    expect(onLaunch).toHaveBeenCalledWith(expect.objectContaining({ name: "auth-worker", count: 3 }));
  });

  it("calls delete_template invoke when delete clicked", async () => {
    mockInvoke.mockResolvedValueOnce(mockTemplates).mockResolvedValueOnce([mockTemplates[1]]);
    render(<TemplatesPanel open onClose={vi.fn()} onLaunch={vi.fn()} />);
    await waitFor(() => screen.getAllByRole("button", { name: /delete/i }));
    fireEvent.click(screen.getAllByRole("button", { name: /delete/i })[0]);
    await waitFor(() => expect(mockInvoke).toHaveBeenCalledWith("delete_template", { templateId: "t1" }));
  });
  ...
  it("does not render when closed", () => {
    const { container } = render(<TemplatesPanel open={false} onClose={vi.fn()} onLaunch={vi.fn()} />);
